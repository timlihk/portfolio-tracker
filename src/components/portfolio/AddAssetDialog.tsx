...
type AddAssetDialogProps<T extends Record<string, unknown>> = {
  open: boolean;
  ...
  if (isAddingNew) {
    return (
      <div className="flex gap-2">
        ...
        />
        <Button type="button" onClick={handleAddNew} className="h-11">
          Add
        </Button>
        <Button
          type="button"
...
export default function AddAssetDialog<T extends Record<string, unknown>>({
  open,
  onOpenChange,
  title,
  fields,
            ...
            {fields.map((field) => (
              <div key={field.name} className="space-y-2">
                ...
                {field.type === "select" && field.allowCustom ? (
                  <SelectWithCustom
                    value={(data[field.name] as string) || undefined}
